...
$("logoutButton")?.addEventListener("click", async () => { await signOut(auth); window.location.href = "../login.html"; });
...
async function loadDashboard(user) {
  ...
  try {
    ...
    if (!attemptsSnap.empty) results.innerHTML = attemptsSnap.docs.map((item) => { const data = item.data(); return `<div class="result-item"><strong>${data.quizTitle || "Quiz"}</strong><span>${data.score ?? "Pending"} pts · ${data.percentage ?? "—"}%</span></div>`; }).join("");
  } catch (error) { console.warn("Recent results lookup failed", error); }

  try {
    const announcementsSnap = await getDocs(query(collection(db, "announcements"), where("active", "==", true), orderBy("createdAt", "desc"), limit(3)));
...
onAuthStateChanged(auth, async (user) => {
  ...
  try { await loadDashboard(user); } catch (error) { console.error(error); showError(error.message === "student-profile-missing" ? "Your student profile could not be found. Please sign out and register again." : "We couldn't load your dashboard. Please try again."); }
});
